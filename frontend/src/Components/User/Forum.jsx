...
const UserForum = () => {
  ...
  const [showReplyInput, setShowReplyInput] = useState(null); // { postId, commentId, userName }
  ...
  const fetchPosts = async (pageNum = 1) => {
    try {
      ...
      // Fetch avatars for all users in posts
    ...
    try {
      ...
      for (const userId of userIds) {
        try {
          ...
          // User may not have avatar or endpoint not available
        }
      }
  ...
  const fetchUserAvatar = async (userId) => {
    ...
    try {
      ...
      // User may not have avatar
    }
    return null;
  };

  // Get user avatar URL - FIXED
  const getUserAvatarUrl = (author) => {
    if (!author) return null;
    
    // First check if we have the avatar in our map
    if (author.userId && userAvatars[author.userId]) {
      return userAvatars[author.userId];
    }
    
    // Check if the author object has avatar data
    ...
    // If it's the current user, check localStorage
  ...
  const handleCreatePost = async (e) => {
    ...
    try {
      ...
      // Fetch updated posts to get avatars
  ...
  const fetchPostById = async (postId) => {
    try {
      ...
      // Fetch avatars for the post's authors
  ...
  // Add reply to comment (Facebook-style)
  ...
  // Build comment tree (Facebook-style nested replies)
  const buildCommentTree = (comments) => {
    ...
    // Sort replies by createdAt (oldest first for threads)
  ...
  // Render comment with nested replies (Facebook-style)
    ...
    return (
      <div key={comment._id || comment.createdAt} className={`${isReply ? 'ml-8 mt-2' : 'mt-3'}`}>
        <div className="flex gap-3">
          {/* Avatar */}
          <div className="w-8 h-8 rounded-full bg-emerald-600 flex items-center justify-center text-white font-semibold text-xs flex-shrink-0 overflow-hidden">
            {avatarUrl ? (
              <img 
                src={avatarUrl} 
                ...
                onError={(e) => {
                  ...
                  e.target.parentElement.textContent = getInitial(comment.author.name);
                }}
              />
            ) : (
          ...
          <div className="flex-1 bg-gray-50 p-3 rounded-lg">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {comment.replyToUserName && (
                  <span className="text-xs text-gray-500">
                    <span className="text-blue-600">@{comment.replyToUserName}</span>
                  </span>
                )}
              </div>
            ...
            {/* Reply input for this comment - Facebook style inline */}
            {showReplyInput && showReplyInput.commentId === comment._id && showReplyInput.postId === post._id && (
              <div className="mt-3 pt-2 border-t border-gray-200">
                <div className="flex gap-2">
                  <div className="w-7 h-7 rounded-full bg-emerald-600 flex items-center justify-center text-white font-semibold text-xs flex-shrink-0 overflow-hidden">
                    {getUserAvatarUrl(user) ? (
                      <img 
                        ...
                        onError={(e) => {
                          e.target.style.display = 'none';
                          e.target.parentElement.textContent = getInitial(user?.name);
                        }}
                      />
                    ) : (
  ...
  const getAvatarDisplay = (author, fallbackName) => {
    const avatarUrl = getUserAvatarUrl(author);
    if (avatarUrl) {
      return (
        <img 
          src={avatarUrl} 
          alt={author?.name || fallbackName} 
          className="w-full h-full object-cover"
          onError={(e) => {
            e.target.style.display = 'none';
            e.target.parentElement.textContent = getInitial(author?.name || fallbackName);
          }}
        />
      );
    }
    return getInitial(author?.name || fallbackName);
  };
      ...
      <div className="flex-1 max-w-4xl w-full mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-xl shadow-md p-4 mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-full bg-emerald-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0 overflow-hidden">
              {getUserAvatarUrl(user) ? (
                <img 
                  src={getUserAvatarUrl(user)} 
                  alt={user.name}
              ...
              <form onSubmit={handleCreatePost} className="space-y-4" encType="multipart/form-data">
                ...
                <div>
                  <input
                    ...
                    disabled={uploading}
                  />
                  <p className="text-xs text-gray-500 mt-1">Upload images or videos (max 10MB each)</p>
                </div>
                  ...
                  <div className="w-10 h-10 rounded-full bg-emerald-600 flex items-center justify-center text-white font-semibold text-sm flex-shrink-0 overflow-hidden">
                    {postAvatarUrl ? (
                      <img 
                        ...
                        onError={(e) => {
                          e.target.style.display = 'none';
                          e.target.parentElement.textContent = getInitial(post.author.name);
                        }}
                      />
                    ) : (
